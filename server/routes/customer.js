...
// @route   POST /api/customer/account/deactivate
// @access  Private
router.post('/account/deactivate', asyncHandler(async (req, res) => {
  const userId = req.user.id;
  const { token } = req.body;

  // TODO: Verify token from email confirmation
  // For now, we'll proceed with deactivation
  ...
  try {
    ...
    user.deactivationReason = req.body.reason || 'User requested';
    
    await user.save();

    // TODO: Send confirmation email
    // TODO: Clear active sessions
    // TODO: Hide user from all public listings

    res.json({
      success: true,
      message: 'Account has been deactivated successfully. You can reactivate by logging in again.',
      data: {
...
// @route   POST /api/customer/account/reactivate
// @access  Public (requires valid credentials)
router.post('/account/reactivate', asyncHandler(async (req, res) => {
...
// @desc    Request permanent account deletion
// @route   POST /api/customer/account/delete-request
// @access  Private
router.post('/account/delete-request', asyncHandler(async (req, res) => {
  const userId = req.user.id;
  ...
  try {
    ...
    // Email should contain: ${process.env.FRONTEND_URL}/confirm-deletion?token=${deletionToken}

    res.json({
      success: true,
      message: 'Deletion confirmation email sent. Please check your email to confirm permanent account deletion.',
      data: {
        email: user.email,
...
// @desc    Confirm and execute permanent account deletion
// @route   POST /api/customer/account/delete-confirm
// @access  Public (requires valid token)
router.post('/account/delete-confirm', asyncHandler(async (req, res) => {
  const { token } = req.body;
  ...
  try {
    ...
    await Promise.all([
      ...
      // - Subscriptions
      // - Payment records (keep for legal compliance)
