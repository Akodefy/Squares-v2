...
class AdminMessageService {
  ...
  async searchUsers(query: string, excludeRoles: string[] = ['admin', 'superadmin', 'subadmin']): Promise<Array<{
    _id: string;
    email: string;
    role: string;
    profile?: {
    ...
    try {
      ...
      }>(`/admin/users/search?q=${encodeURIComponent(query)}&excludeRoles=${excludeRoles.join(',')}`);
